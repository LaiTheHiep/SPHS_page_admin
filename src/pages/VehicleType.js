...
class VehicleType extends React.Component {
  ...
  get(_query) {
    BaseAction.get(db_collection.vehicleTypes, {})
      .then((res) => {
  ...
  post(_body) {
    BaseAction.post(db_collection.vehicleTypes, _body)
      .then((res) => {
        if (res.data.errorMessage) {
  ...
  put(_body) {
    BaseAction.put(db_collection.vehicleTypes, _body)
      .then((res) => {
        if (res.data.errorMessage) {
          alert(`${res.data.errorName}: ${res.data.errorMessage}`);
          return;
  ...
  delete(id) {
    ...
    if (id) {
      BaseAction.delete(db_collection.vehicleTypes, id)
        .then((res) => {
          if (res.data.errorMessage) {
            alert(`${res.data.errorName}: ${res.data.errorMessage}`);
            return;
  ...
  render() {
    ...
    return (
      <div>
        <Row>
          <Button color='success' onClick={this.toggleCreate}>+ Create new vehicle type</Button>
        </Row>
        <br />
        <Row>
          <Table
        ...
        <Modal isOpen={this.state.isOpen} toggle={this.isOpen} size='lg'>
          <ModalHeader>{this.state.typeSubmit === 1 ? 'Create new item' : 'Edit item'}</ModalHeader>
          <ModalBody>
            <Row>
